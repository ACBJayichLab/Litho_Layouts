...
class MaskDesigner:
    ...
    def generate_design(self, output_dir="output"):
        ...
        # Set layer names in the layout using LayerInfo with name parameter
        # This embeds the name in the GDS file (supported by KLayout and some tools)
        for (layer_num, datatype), name in self.mask_config.LAYER_NAMES.items():
            layer_idx = self.layout.layer(layer_num, datatype)
            # Create new LayerInfo with layer, datatype, AND name
            named_layer_info = pya.LayerInfo(layer_num, datatype, name)
            self.layout.set_info(layer_idx, named_layer_info)
