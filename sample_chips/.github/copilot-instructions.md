...
```bash
python 100mm_wafer_sample_omegas_maskV2.py  # Outputs _inspect.gds and _prod.gds to output/ folder
```
...
- _prod.gds should be made directly from _inspect.gds
- Design edits should not be done to _prod.gds
...
```
100mm_wafer_sample_omegas_maskV2.py
├─ ChipConfig         # Per-chip parameters (aperture, DC pads, traces)
├─ OmegaConfig        # Omega resonator parameters (radius, trace width, gap)
├─ MaskConfig         # Wafer/mask-level parameters (dimensions, layers, DBU)
├─ MaskDesigner       # Main design generator class
│  ├─ create_wafer_region()
│  ├─ create_omega()
│  ├─ create_dc_wire_from_cpw()
│  ├─ create_unit_cell()
│  └─ generate_design()
└─ main()             # Entry point

6x6mm_sample_chip_V2.py
├─ DesignConfig       # All chip parameters (dimensions, layers, DBU)
├─ ChipDesigner       # Chip-level design generator
│  ├─ create_bond_pad()
│  ├─ create_dc_pad_array()
│  ├─ create_ground_plane_with_traces()
│  └─ create_chip()
└─ create_chip_cell() # Factory function for chip generation
```
