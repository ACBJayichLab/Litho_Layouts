...
class DesignConfig:
    ...
    prt_y_offset = 1500.0               # Y offset from chip center (µm) - place at center for now
...
class ChipDesigner:
    ...
    def create_alignment_cross(self, cell, x_center, y_center, config=None):
        ...
        Also creates DC pad layer and PRT layer extension rectangles at the ends 
        of each arm to enable alignment between the layers.
        ...
        # PRT layer extensions at ends of each arm (for layer alignment) - identical to DC layer
        prt_layer_idx = self.layout.layer(config.PRT_LAYER)
        
        # Left extension
        cell.shapes(prt_layer_idx).insert(left_ext)
        
        # Right extension
        cell.shapes(prt_layer_idx).insert(right_ext)
        
        # Bottom extension
        cell.shapes(prt_layer_idx).insert(bottom_ext)
        
        # Top extension
        cell.shapes(prt_layer_idx).insert(top_ext)
    ...
    def create_prt(self, cell, y_offset, config=None):
        """
        Create a Platinum Resistance Thermometer (PRT) serpentine pattern.
        
        The PRT consists of:
        - Two square bond pads at left and right ends
        - A serpentine (meander) trace running back and forth between pads
        
        The meander long direction is horizontal (X). Traces run left-to-right,
        starting from the top of the left pad and ending at the bottom of the right pad.
        Horizontal runs connected by short vertical segments at alternating ends.
        
        Args:
            cell: KLayout cell to insert PRT into
            y_offset: vertical offset from chip center (positive = above, negative = below)
            config: design configuration
        
        Returns:
            pya.Region representing the PRT geometry (for clearance calculations if needed)
        """
        if config is None:
            config = self.config
        
        # Check if PRT is enabled
        if not getattr(config, 'prt_enabled', True):
            return pya.Region()
        
        chip_center_x = config.chip_width / 2.0
        chip_center_y = config.chip_height / 2.0
        
        # PRT region center (relative to chip center)
        prt_center_x = chip_center_x + config.prt_x_offset
        prt_center_y = chip_center_y + y_offset
        
        # PRT region bounds (long direction is X = prt_region_height, short direction is Y = prt_region_width)
        region_left = prt_center_x - config.prt_region_height / 2.0
        region_right = prt_center_x + config.prt_region_height / 2.0
        region_bottom = prt_center_y - config.prt_region_width / 2.0
        region_top = prt_center_y + config.prt_region_width / 2.0
        
        # Pad positions (at left/right edges, vertically centered)
        pad_size = config.prt_pad_size
        trace_width = config.prt_trace_width
        trace_gap = config.prt_trace_gap  # centerline-to-centerline gap
        
        # Left pad center
        left_pad_center_x = region_left + pad_size / 2.0
        left_pad_center_y = prt_center_y
        
        # Right pad center
        right_pad_center_x = region_right - pad_size / 2.0
        right_pad_center_y = prt_center_y
        ...
        # Create left pad
        left_pad = pya.Box(
            self._um_to_dbu(left_pad_center_x - pad_size / 2.0),
            self._um_to_dbu(left_pad_center_y - pad_size / 2.0),
            self._um_to_dbu(left_pad_center_x + pad_size / 2.0),
            self._um_to_dbu(left_pad_center_y + pad_size / 2.0)
        )
        cell.shapes(prt_layer_idx).insert(left_pad)
        
        # Create right pad
        right_pad = pya.Box(
            self._um_to_dbu(right_pad_center_x - pad_size / 2.0),
            self._um_to_dbu(right_pad_center_y - pad_size / 2.0),
            self._um_to_dbu(right_pad_center_x + pad_size / 2.0),
            self._um_to_dbu(right_pad_center_y + pad_size / 2.0)
        )
        cell.shapes(prt_layer_idx).insert(right_pad)
        
        # Serpentine trace region
        # Trace starts from top of left pad (right edge) and ends at bottom of right pad (left edge)
        # Add gap between meander and pads to prevent shorting
        pad_to_trace_gap = trace_gap  # Gap between pad edge and first/last meander trace
        serpentine_left_x = left_pad_center_x + pad_size / 2.0 + pad_to_trace_gap
        serpentine_right_x = right_pad_center_x - pad_size / 2.0 - pad_to_trace_gap
        
        # Y positions: start at top of left pad, end at bottom of right pad
        start_y = left_pad_center_y + pad_size / 2.0 - trace_width / 2.0  # top of left pad
        end_y = right_pad_center_y - pad_size / 2.0 + trace_width / 2.0    # bottom of right pad
        
        # Calculate number of horizontal runs that fit in the vertical space
        # Meander goes from start_y down to end_y
        available_height = start_y - end_y  # positive value (top to bottom)
        num_runs = int(available_height / trace_gap) + 1
        
        # Ensure odd number of runs so trace ends on right side
        if num_runs % 2 == 0:
            num_runs += 1
        if num_runs < 1:
            num_runs = 1
        
        # Recalculate actual gap to evenly distribute runs
        if num_runs > 1:
            actual_gap = available_height / (num_runs - 1)
        else:
            actual_gap = 0
        
        # Build serpentine path
        prt_region = pya.Region()
        
        # Create horizontal runs and vertical connections
        for i in range(num_runs):
            run_y = start_y - i * actual_gap
            
            # Determine direction: even runs go left-to-right (full width), odd go right-to-left
            if i % 2 == 0:
                # Even run: left to right (full width)
                run_left = serpentine_left_x
                run_right = serpentine_right_x
            else:
                # Odd run: right to left (full width)
                run_left = serpentine_left_x
                run_right = serpentine_right_x
            
            # Create horizontal run
            horiz_run = pya.Box(
                self._um_to_dbu(run_left),
                self._um_to_dbu(run_y - trace_width / 2.0),
                self._um_to_dbu(run_right),
                self._um_to_dbu(run_y + trace_width / 2.0)
            )
            prt_region.insert(horiz_run)
            
            # Create vertical connection to next run (if not last run)
            if i < num_runs - 1:
                next_run_y = start_y - (i + 1) * actual_gap
                if i % 2 == 0:
                    # Connect at right edge (going down)
                    vert_conn = pya.Box(
                        self._um_to_dbu(serpentine_right_x - trace_width),
                        self._um_to_dbu(next_run_y - trace_width / 2.0),
                        self._um_to_dbu(serpentine_right_x),
                        self._um_to_dbu(run_y + trace_width / 2.0)
                    )
                else:
                    # Connect at left edge (going down)
                    vert_conn = pya.Box(
                        self._um_to_dbu(serpentine_left_x),
                        self._um_to_dbu(next_run_y - trace_width / 2.0),
                        self._um_to_dbu(serpentine_left_x + trace_width),
                        self._um_to_dbu(run_y + trace_width / 2.0)
                    )
                prt_region.insert(vert_conn)
        
        # Start connection: from top of left pad to first horizontal run
        # Vertical segment up from pad, then horizontal to serpentine area
        left_pad_top_y = left_pad_center_y + pad_size / 2.0
        left_pad_right_x = left_pad_center_x + pad_size / 2.0
        
        # Vertical from top of left pad
        start_vert = pya.Box(
            self._um_to_dbu(left_pad_right_x - trace_width),
            self._um_to_dbu(left_pad_top_y),
            self._um_to_dbu(left_pad_right_x),
            self._um_to_dbu(start_y + trace_width / 2.0)
        )
        prt_region.insert(start_vert)
        
        # Horizontal from pad edge to serpentine left edge
        start_horiz = pya.Box(
            self._um_to_dbu(left_pad_right_x - trace_width),
            self._um_to_dbu(start_y - trace_width / 2.0),
            self._um_to_dbu(serpentine_left_x + trace_width),
        ...
        prt_region.insert(start_horiz)
        
        # End connection: from last horizontal run to bottom of right pad
        last_run_y = start_y - (num_runs - 1) * actual_gap
        right_pad_bottom_y = right_pad_center_y - pad_size / 2.0
        right_pad_left_x = right_pad_center_x - pad_size / 2.0
        
        # Last run ends on right side (odd index means left edge, even means right edge)
        if (num_runs - 1) % 2 == 0:
            # Ends on right side - connect from serpentine to pad
            # Horizontal from serpentine right edge to pad
            end_horiz = pya.Box(
                self._um_to_dbu(serpentine_right_x - trace_width),
                self._um_to_dbu(last_run_y - trace_width / 2.0),
                self._um_to_dbu(right_pad_left_x + trace_width),
                self._um_to_dbu(last_run_y + trace_width / 2.0)
            )
            prt_region.insert(end_horiz)
            
            # Vertical down to bottom of right pad
            end_vert = pya.Box(
                self._um_to_dbu(right_pad_left_x),
                self._um_to_dbu(right_pad_bottom_y),
                self._um_to_dbu(right_pad_left_x + trace_width),
                self._um_to_dbu(last_run_y + trace_width / 2.0)
            )
            prt_region.insert(end_vert)
        else:
            # Ends on left side - need extra segment to get to right pad
            # Vertical down on left side, then horizontal across, then vertical to pad
            # This case: add vertical at left, horizontal to right, then vertical to pad
            end_vert_left = pya.Box(
                self._um_to_dbu(serpentine_left_x),
                self._um_to_dbu(end_y - trace_width / 2.0),
                self._um_to_dbu(serpentine_left_x + trace_width),
                self._um_to_dbu(last_run_y + trace_width / 2.0)
            )
            prt_region.insert(end_vert_left)
            
            # Horizontal at bottom to right pad area
            end_horiz_bottom = pya.Box(
                self._um_to_dbu(serpentine_left_x),
                self._um_to_dbu(end_y - trace_width / 2.0),
                self._um_to_dbu(right_pad_left_x + trace_width),
                self._um_to_dbu(end_y + trace_width / 2.0)
            )
            prt_region.insert(end_horiz_bottom)
        ...
        # Merge all shapes and insert
        prt_region.merge()
        cell.shapes(prt_layer_idx).insert(prt_region)
    ...
    def create_chip(self, chip_name="chip_main", config=None, label=None):
        ...
        # 7.5. Create Platinum Resistance Thermometers (PRT) if enabled - top and bottom
        if getattr(config, 'prt_enabled', True):
            self.create_prt(chip_cell, config.prt_y_offset, config)   # Top PRT
            self.create_prt(chip_cell, -config.prt_y_offset, config)  # Bottom PRT
