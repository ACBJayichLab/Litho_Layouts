...
class ChipConfig:
    ...
    PRT_LAYER = pya.LayerInfo(4, 0)    # Platinum resistance thermometer layer (prt 4/0)
    ...
    dc_wire_width = 25.0             # Width of DC wire (µm) - same as electroplating_tab_width
    ...
    # Platinum Resistance Thermometer (PRT) parameters
    prt_enabled = True               # Enable PRT feature
    prt_pad_size = 125.0             # PRT bond pad size (square, µm)
    prt_trace_width = 10.0           # PRT serpentine trace width (µm)
    prt_trace_gap = 20.0             # Centerline-to-centerline gap between traces (µm)
    prt_region_width = 300.0         # Width of PRT region (µm)
    prt_region_height = 1600.0       # Height of PRT region (µm)
    prt_x_offset = 0.0               # X offset from chip center (µm)
    prt_y_offset = 1575.0            # Y offset from chip center (µm)
...
class MaskConfig:
    ...
    METAL_LAYER = pya.LayerInfo(1, 0)  # Metal/conductor layer (CPW 1/0)
    GUIDE_LAYER = pya.LayerInfo(2, 0)  # Guide/alignment layer (mask_border 2/0)
    DC_PAD_LAYER = pya.LayerInfo(3, 0)  # DC pads and traces layer (dc_contacts 3/0)
    PRT_LAYER = pya.LayerInfo(4, 0)    # Platinum resistance thermometer layer (prt 4/0)
    INTERCONNECT_LAYER = pya.LayerInfo(5, 0)  # Chip interconnect layer (interconnect 5/0)
    ...
    # Chip interconnect layer parameters
    interconnect_enabled = True       # Enable chip interconnect layer
    interconnect_trace_width = 500.0  # Width of interconnect traces (µm) - wider than electroplating
...
class MaskDesigner:
    ...
    def __init__(self, mask_config=None, chip_configs=None, omega_configs=None):
        ...
        # Pre-register all layers with names upfront
        # This ensures layer names are embedded in the GDS file
        # Using layout.layer(layer, datatype, name) as per KLayout documentation
        self.layout.layer(1, 0, "CPW 1/0")
        self.layout.layer(2, 0, "mask_border 2/0")
        self.layout.layer(3, 0, "dc_contacts 3/0")
        self.layout.layer(4, 0, "prt 4/0")
        self.layout.layer(5, 0, "interconnect 5/0")
    ...
    def create_dc_wire_from_cpw(self, cell, x_center, y_center, chip_config):
        ...
        The wire is placed on CPW layer (1/0), PRT layer (4/0), and interconnect layer (5/0),
        extending from the right bond pad outward across the gap into the nearby ground plane.
        ...
        prt_layer_idx = self.layout.layer(config.PRT_LAYER)
        interconnect_layer_idx = self.layout.layer(config.INTERCONNECT_LAYER)
        ...
        # Create the wire rectangle on CPW layer (1/0)
        wire_box = pya.Box(
        ...
        # Create identical wire on PRT layer (4/0)
        cell.shapes(prt_layer_idx).insert(wire_box)
        
        # Create identical wire on interconnect layer (5/0)
        cell.shapes(interconnect_layer_idx).insert(wire_box)
        
        print(f"  → DC wire: {wire_width:.0f} µm wire at x={wire_start_x:.0f} to {wire_end_x:.0f} µm, y={y_center:.0f} µm, length {wire_length:.0f} µm (CPW + PRT + interconnect layers)")
    ...
    def create_unit_cell(self):
        ...
        for row in range(config.chip_count_y):
            for col in range(config.chip_count_x):
                ...
                # Add DC wire from right CPW line to ground plane for all chips
                # (electroplating tab between right bond pad and ground)
    ...
    def create_alignment_cross(self, cell, x_center, y_center, size=500.0, linewidth=50.0):
        """
        Create an alignment cross (+ shape) at specified position on all layers.
        ...
        cell.shapes(prt_layer_idx).insert(h_bar)
        cell.shapes(prt_layer_idx).insert(v_bar)
    ...
    def create_mask_labels(self, cell, design_name="100mm_wafer_sample_omegas_V3"):
        """
        Create text labels as polygons in the top left corner of the mask plate.
        Adds design name, date, and layer info labels on the CPW (metal), 
        DC trace, PRT, and interconnect layers for identification. Text is converted
        ...
        # PRT Layer labels (Layer 3/4)
        prt_label_lines = [
            design_name,
            date_str,
            "Layer 3/4 - Pt Thermometer ~1kOhm @ 4K",
            "Jayich Lab - Jeff Ahlers"
        ]
        current_y = label_y
        for line in prt_label_lines:
            self.text_to_polygons(line, label_x, current_y, text_height, prt_layer_idx, cell)
            current_y -= (text_height + line_spacing)
        
        # Interconnect Layer labels (Layer 4/4)
        interconnect_label_lines = [
            design_name,
            date_str,
            "Layer 4/4 - Interconnect fix",
            "Jayich Lab - Jeff Ahlers"
        ]
        current_y = label_y
        for line in interconnect_label_lines:
            self.text_to_polygons(line, label_x, current_y, text_height, interconnect_layer_idx, cell)
            current_y -= (text_height + line_spacing)
        
        print(f"  Added mask labels (as polygons): '{design_name}', '{date_str}', layer info on CPW, DC, PRT, and interconnect layers")
    ...
    def create_interconnect_layer(self, cell, chip_positions):
        """
        Create chip interconnect layer with wider traces, wafer edge, and alignment features.
        
        This layer includes:
        - Wider dicing lane traces (interconnect_trace_width)
        - Wafer edge fill (same as metal layer)
        - Studded alignment ring
        - Corner alignment marks
        - Electroplating contact bars
        
        Args:
            cell: KLayout cell to insert features into
            chip_positions: List of (chip_x, chip_y, variant_index) tuples
        """
        config = self.mask_config
        base_chip_config = self.chip_configs[0]
        ...
        chip_width = base_chip_config.chip_width
        chip_height = base_chip_config.chip_height
        trace_width = config.interconnect_trace_width
        usable_radius = (config.wafer_diameter - 2 * config.wafer_buffer) / 2.0
        flat_exclusion_zone = 2250.0
        flat_y = -config.wafer_radius + config.flat_depth
        
        print("Creating interconnect layer...")
        
        # === WAFER EDGE FILL ===
        # Copy the wafer edge fill region to interconnect layer
        wafer_region = self.create_wafer_region()
        
        # Create usable region (inner circle)
        num_segments = 360
        usable_points = []
        for i in range(num_segments):
            angle = 2.0 * math.pi * i / num_segments
        ...
        # Edge fill = full wafer - usable area
        edge_fill_region = wafer_region - usable_region
        cell.shapes(interconnect_layer_idx).insert(edge_fill_region)
        print("  Added wafer edge fill to interconnect layer")
        
        # === ELECTROPLATING CONTACT BARS ===
        # Similar to create_electroplating_contact_bars but for interconnect layer
        min_chip_x = min(pos[0] for pos in chip_positions)
        max_chip_x = max(pos[0] for pos in chip_positions) + chip_width
        min_chip_y = min(pos[1] for pos in chip_positions)
        max_chip_y = max(pos[1] for pos in chip_positions) + chip_height
        
        # Bar dimensions - 4 dies wide for larger contact area in tangential direction
        bar_width = 4 * chip_width
        
        # Top bar
        top_bar = pya.Box(
            self._um_to_dbu(-bar_width / 2.0),
            self._um_to_dbu(max_chip_y),
            self._um_to_dbu(bar_width / 2.0),
            self._um_to_dbu(usable_radius)
        )
        cell.shapes(interconnect_layer_idx).insert(top_bar)
        
        # Left bar
        left_bar = pya.Box(
            self._um_to_dbu(-usable_radius),
            self._um_to_dbu(-bar_width / 2.0),
            self._um_to_dbu(min_chip_x),
            self._um_to_dbu(bar_width / 2.0)
        )
        cell.shapes(interconnect_layer_idx).insert(left_bar)
        
        # Right bar
        right_bar = pya.Box(
            self._um_to_dbu(max_chip_x),
            self._um_to_dbu(-bar_width / 2.0),
            self._um_to_dbu(usable_radius),
            self._um_to_dbu(bar_width / 2.0)
        )
        cell.shapes(interconnect_layer_idx).insert(right_bar)
        ...
        bottom_bar = pya.Box(
            ...
            self._um_to_dbu(flat_y),
            self._um_to_dbu(bar_width / 2.0),
            self._um_to_dbu(min_chip_y)
        )
        cell.shapes(interconnect_layer_idx).insert(bottom_bar)
        print("  Added electroplating contact bars to interconnect layer")
        
        # === STUDDED ALIGNMENT RING ===
        # Create studded ring at wafer edge (100mm diameter)
        ring_diameter = 100.0  # mm
        ring_radius = (ring_diameter * 1000.0) / 2.0  # µm
        stud_width = 500.0
        stud_spacing = 1000.0
        ring_width = 200.0
        
        circumference = 2.0 * math.pi * ring_radius
        stud_pitch = stud_width + stud_spacing
        num_segments_ring = int(circumference / stud_pitch)
        
        inner_radius = ring_radius - ring_width / 2.0
        outer_radius = ring_radius + ring_width / 2.0
        
        for i in range(num_segments_ring):
            start_angle = (i * stud_pitch) / ring_radius
            end_angle = start_angle + stud_width / ring_radius
            ...
            for j in range(num_arc_points + 1):
                ...
                x = self._um_to_dbu(outer_radius * math.cos(angle))
                y = self._um_to_dbu(outer_radius * math.sin(angle))
                arc_points.append(pya.Point(x, y))
            
            # Inner arc (CW, reverse)
            for j in range(num_arc_points, -1, -1):
                angle = start_angle + (end_angle - start_angle) * j / num_arc_points
                x = self._um_to_dbu(inner_radius * math.cos(angle))
                y = self._um_to_dbu(inner_radius * math.sin(angle))
                arc_points.append(pya.Point(x, y))
            
            stud_polygon = pya.Polygon(arc_points)
            cell.shapes(interconnect_layer_idx).insert(stud_polygon)
        
        print(f"  Added studded alignment ring to interconnect layer")
        
        # === CORNER ALIGNMENT MARKS ===
        # Create alignment crosses at same positions as other layers (±33000, ±30000)
        positions = [
            (-33000.0, 30000.0),   # Top-left
            (33000.0, 30000.0),    # Top-right
            (-33000.0, -30000.0),  # Bottom-left
        ...
        for x_center, y_center in positions:
            # Horizontal bar
            h_bar = pya.Box(
                self._um_to_dbu(x_center - cross_size / 2.0),
                self._um_to_dbu(y_center - linewidth / 2.0),
                self._um_to_dbu(x_center + cross_size / 2.0),
                self._um_to_dbu(y_center + linewidth / 2.0)
            )
            cell.shapes(interconnect_layer_idx).insert(h_bar)
            
            # Vertical bar
            v_bar = pya.Box(
                self._um_to_dbu(x_center - linewidth / 2.0),
                self._um_to_dbu(y_center - cross_size / 2.0),
                self._um_to_dbu(x_center + linewidth / 2.0),
                self._um_to_dbu(y_center + cross_size / 2.0)
            )
            cell.shapes(interconnect_layer_idx).insert(v_bar)
        
        print(f"  Added {len(positions)} corner alignment marks to interconnect layer")
        
        # === WIDER DICING LANE TRACES ===
        # Collect unique x and y coordinates for dicing lanes
        x_coords = set()
        y_coords = set()
        
        # Extension beyond usable radius into the edge ring
        grid_extension = 100.0  # µm extension into edge fill region
        ...
        for x in sorted(x_coords):
            if abs(x) < usable_radius:
                y_extent = math.sqrt(usable_radius * usable_radius - x * x) + grid_extension
                y_bottom = max(-y_extent, flat_y + flat_exclusion_zone)
                y_top = y_extent
                
                if y_bottom < y_top:
                    trace_box = pya.Box(
                        self._um_to_dbu(x - trace_width / 2.0),
                        self._um_to_dbu(y_bottom),
                        self._um_to_dbu(x + trace_width / 2.0),
                        self._um_to_dbu(y_top)
                    )
                    cell.shapes(interconnect_layer_idx).insert(trace_box)
        
        # Create horizontal traces at each unique y coordinate
        for y in sorted(y_coords):
            if y >= flat_y + flat_exclusion_zone:
                if abs(y) < usable_radius:
                    x_extent = math.sqrt(usable_radius * usable_radius - y * y) + grid_extension
                    
                    trace_box = pya.Box(
                        self._um_to_dbu(-x_extent),
                        self._um_to_dbu(y - trace_width / 2.0),
                        self._um_to_dbu(x_extent),
                        self._um_to_dbu(y + trace_width / 2.0)
                    )
                    cell.shapes(interconnect_layer_idx).insert(trace_box)
        
        print(f"  Added wider dicing lane traces ({trace_width} µm) to interconnect layer")
        
        # === REMOVE SQUARES AT CHIP CORNER JUNCTIONS ===
        # Create 200 µm squares centered at each intersection of dicing lanes
        junction_square_size = 200.0  # µm
        half_square = junction_square_size / 2.0
        
        # Get all shapes on interconnect layer and convert to region for Boolean operations
        interconnect_region = pya.Region(cell.shapes(interconnect_layer_idx))
        
        # Create region of all junction squares to subtract
        junction_squares_region = pya.Region()
        junction_count = 0
        
        for x in x_coords:
            for y in y_coords:
                # Only add squares that are within the usable wafer area
                dist = math.sqrt(x * x + y * y)
                if dist < usable_radius + grid_extension and y >= flat_y + flat_exclusion_zone:
                    square_box = pya.Box(
                        self._um_to_dbu(x - half_square),
                        self._um_to_dbu(y - half_square),
                        self._um_to_dbu(x + half_square),
                        self._um_to_dbu(y + half_square)
                    )
                    junction_squares_region.insert(square_box)
                    junction_count += 1
        
        # Subtract junction squares from interconnect layer
        interconnect_region -= junction_squares_region
        
        # Clear and re-insert the modified region
        cell.shapes(interconnect_layer_idx).clear()
        cell.shapes(interconnect_layer_idx).insert(interconnect_region)
        
        print(f"  Removed {junction_count} junction squares ({junction_square_size} µm) from interconnect layer")
    ...
    def create_full_wafer_layout(self, wafer_cell):
        ...
        # Add interconnect layer features (wider traces, wafer edge, alignment)
        if config.interconnect_enabled:
            self.create_interconnect_layer(wafer_cell, chip_positions)
    ...
    def _write_layer_properties_file(self, filepath):
        """
        Write a KLayout layer properties file (.lyp) for display configuration.
        
        This file defines how layers are displayed in KLayout, including:
        - Layer names
        - Colors and fill patterns
        - Visibility settings
        
        Args:
            filepath: Output path for the .lyp file
        """
        # Define colors for each layer (RGB hex)
        layer_colors = {
            (1, 0): "#ff0000",  # CPW - Red
            (2, 0): "#00ff00",  # mask_border - Green  
            (3, 0): "#0000ff",  # dc_contacts - Blue
            (4, 0): "#ff00ff",  # prt - Magenta
            (5, 0): "#ffa500",  # interconnect - Orange
        }
        
        lyp_content = '<?xml version="1.0" encoding="utf-8"?>\n'
        lyp_content += '<layer-properties>\n'
        
        for (layer_num, datatype), name in self.mask_config.LAYER_NAMES.items():
            color = layer_colors.get((layer_num, datatype), "#808080")
            lyp_content += f' <properties>\n'
            lyp_content += f'  <frame-color>{color}</frame-color>\n'
            lyp_content += f'  <fill-color>{color}</fill-color>\n'
            lyp_content += f'  <frame-brightness>0</frame-brightness>\n'
            lyp_content += f'  <fill-brightness>0</fill-brightness>\n'
            lyp_content += f'  <dither-pattern>I5</dither-pattern>\n'
            lyp_content += f'  <line-style/>\n'
            lyp_content += f'  <valid>true</valid>\n'
            lyp_content += f'  <visible>true</visible>\n'
            lyp_content += f'  <transparent>false</transparent>\n'
            lyp_content += f'  <width>1</width>\n'
            lyp_content += f'  <marked>false</marked>\n'
            lyp_content += f'  <xfill>false</xfill>\n'
            lyp_content += f'  <animation>0</animation>\n'
            lyp_content += f'  <name>{name}</name>\n'
            lyp_content += f'  <source>{layer_num}/{datatype}@1</source>\n'
            lyp_content += f' </properties>\n'
    ...
    def _write_layer_map_file(self, filepath):
        """
        Write a layer map file for lithography tools.
        
        This file format is commonly used by:
        - Heidelberg laser writers
        - JEOL e-beam writers  
        - Various mask writers
        
        Format: layer/datatype : NAME
        
        Args:
            filepath: Output path for the .map file
        """
        map_content = "# Layer Map File\n"
        map_content += "# Generated by KLayout Mask Design Generator\n"
        map_content += "# Format: layer/datatype : layer_name\n"
        map_content += "#\n"
        
        for (layer_num, datatype), name in self.mask_config.LAYER_NAMES.items():
            map_content += f"{layer_num}/{datatype} : {name}\n"
    ...
    def generate_design(self, output_dir=None):
        ...
        # Layer names were pre-registered in __init__ using layout.layer(num, dt, name)
        # This ensures names are embedded in the GDS file from the start
        print("  Layer names embedded in GDS: CPW 1/0, mask_border 2/0, dc_contacts 3/0, prt 4/0, interconnect 5/0")
        ...
        # Generate layer properties file (.lyp) for KLayout display
        lyp_path = os.path.join(output_dir, f"{script_name}.lyp")
        self._write_layer_properties_file(lyp_path)
        print(f"✓ Layer properties file exported: {lyp_path}")
        
        # Generate layer map file for lithography tools
        map_path = os.path.join(output_dir, f"{script_name}.map")
        self._write_layer_map_file(map_path)
        print(f"✓ Layer map file exported: {map_path}")
...
def main():
    ...
    print(f"  Interconnect layer:  {mask_config.interconnect_enabled} ({mask_config.interconnect_trace_width:.0f} µm traces)")
    print()
