...
class MaskDesigner:
    ...
    def create_dc_wire_from_cpw(self, cell, x_center, y_center, chip_config):
        ...
        The wire is placed on both CPW layer (1/0) and PRT layer (4/0), extending 
        from the right bond pad outward across the gap into the nearby ground plane.
        ...
        print(f"  → DC wire: {wire_width:.0f} µm wire at x={wire_start_x:.0f} to {wire_end_x:.0f} µm, y={y_center:.0f} µm, length {wire_length:.0f} µm (CPW + PRT layers)")
    ...
    def create_mask_labels(self, cell, design_name="100mm_wafer_sample_omegas_V2"):
        """
        ...
        DC trace, and PRT layers for identification. Text is converted
        ...
        # DC Layer labels (Layer 2/3)
        dc_label_lines = [
            design_name,
            date_str,
            "Layer 2/3 - DC Contacts",
            "Jayich Lab - Jeff Ahlers"
        ]
        current_y = label_y
        ...
        # PRT Layer labels (Layer 3/3)
        prt_label_lines = [
            design_name,
            date_str,
            "Layer 3/3 - Pt Thermometer ~1kOhm @ 4K",
            "Jayich Lab - Jeff Ahlers"
        ]
        current_y = label_y
        ...
        print(f"  Added mask labels (as polygons): '{design_name}', '{date_str}', layer info on CPW, DC, and PRT layers")
    ...
    def generate_design(self, output_dir=None):
        ...
        print("  Layer names embedded in GDS: CPW 1/0, mask_border 2/0, dc_contacts 3/0, prt 4/0")
